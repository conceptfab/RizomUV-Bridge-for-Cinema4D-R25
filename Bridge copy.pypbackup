...
import os, re, json, subprocess, time, platform, logging, threading
    ...
    def Execute(self, doc):
        ...
        if self.plugin == "export":
            ...
            if bc[c4d.BFM_INPUT_QUALIFIER] == 1:
                dlg = Options()
                dlg.Open(
                    c4d.DLG_TYPE_MODAL, xpos=-1, ypos=-1, defaultw=200, defaulth=100
                )
            else:
                self.dlg = Starter()
...
class Exporter:
    ...
    def dispatch(self, mode):
        prefs_path = storage.GeGetC4DPath(c4d.C4D_PATH_PREFS)
        if not prefs_path or not isinstance(prefs_path, str) or not os.path.exists(prefs_path):
            c4d.gui.MessageDialog("Błąd: Nie można pobrać ścieżki do preferencji C4D!\nPlugin nie może działać.")
            logger.error(f"Nieprawidłowa ścieżka preferencji: {prefs_path}")
            self.plugin_folder = None
            self.settings_path = None
            self.object_path = None
            self.scripts_folder = None
            return
        self.plugin_folder = os.path.join(prefs_path, "rizomUV")
    ...
    def settings_save(self):
        try:
            ...
            logger.error(f"Błąd zapisu ustawień: {str(e)}")
            return False
            ...
            for index, script in enumerate(scripts):
                if self.scripts_folder:
                    file_path = os.path.join(
                        self.scripts_folder, "demo_script_" + str(index) + ".lua"
                    )
                    try:
                        with open(file_path, "w", encoding='utf-8') as file_:
                            file_.write(script)
                        logger.info(f"Utworzono demo skrypt: {file_path}")
                    except (IOError, OSError) as e:
                        logger.error(f"Nie można utworzyć demo skryptu: {str(e)}")
        except Exception as e:
            logger.error(f"Błąd podczas tworzenia demo skryptów: {str(e)}")
    ...
    def script_save(self, name, script, dialog=True, mode="w"):
        ...
        if dialog:
            script_name = storage.SaveDialog(
                0, "Save *.lua File", "lua", def_path=self.scripts_folder or "", def_file=name
            )
        else:
            if self.scripts_folder is None:
                logger.error("Folder skryptów nie jest ustawiony")
                return None
        ...
        if self.scripts_folder and os.path.exists(self.scripts_folder):
            try:
                ...
                logger.error(f"Błąd zapisu pliku: {str(e)}")
                return None
    ...
    def script_formation(self, selection, script):
        if self.object_path:
            ...
            load_string = (
                'ZomLoad({File={Path="'
                + path
                + '", ImportGroups=true, XYZUVW=true, UVWProps='
                + str(self.ui["CHK_NEW_UV"][1])
                + "}, NormalizeUVW=true})"
            )
            ...
            if self.ui["CHK_SINGLE"][1] and len(script) > 0:
                save_string = (
                    'ZomSave({File={Path="'
                    + path
                    + '", UVWProps=true}, __UpdateUIObjFileName=true})'
                )
    ...
    def validate_configuration(self):
        errors = []
        # Sprawdź ścieżki bazowe
        if not self.plugin_folder or not isinstance(self.plugin_folder, str) or not os.path.exists(self.plugin_folder):
            errors.append("Nieprawidłowa ścieżka pluginu (plugin_folder)")
        if not self.scripts_folder or not isinstance(self.scripts_folder, str) or not os.path.exists(self.scripts_folder):
            errors.append("Folder skryptów nie istnieje")
        # Sprawdź ścieżkę do RizomUV
        rizomuv_path = self.ui["TXT_U3D_PATH"][1]
        logger.info(f"Walidacja ścieżki RizomUV: {rizomuv_path}")
        if not rizomuv_path or not isinstance(rizomuv_path, str) or not os.path.exists(rizomuv_path):
...
class Starter(Exporter):
    ...
    def rizomuv_run(self, script, cmd):
        ...
        self.selected_objs = self.doc.GetActiveObjects(
            c4d.GETACTIVEOBJECTFLAGS_CHILDREN
        )

        # Tworzenie backup przed eksportem
        self.create_backup(self.object_path)
        ...
        # [ SELECTION ] - - - - - - - - - - - - - - - - - - - - - - - [/]
        lua_selection = ""
        cmd = False  # Domyślnie nie używamy skryptu
        
        if self.ui["RADIO_GROUP"][1] > 1:

            index = -1
            to_select = []
            has_uv_objects = False
            ...
            for obj in self.selected_objs:
                ...
                # Sprawdź czy obiekt ma tag UV
                uv_tag = obj.GetTag(c4d.Tuvw)
                if uv_tag is None:
                    logger.warning(f"Obiekt {obj.GetName()} nie ma tagu UV - eksportuję bez selekcji krawędzi")
                    continue
            ...
            if to_select and has_uv_objects:
                lua_selection = "edges = {" + ", ".join(to_select) + "}\n" + edge_selection
                cmd = True
            else:
                if not has_uv_objects:
                    logger.warning("Żaden z obiektów nie ma tagu UV - eksportuję bez selekcji krawędzi")
                else:
                    logger.warning("Nie znaleziono żadnych krawędzi do wyboru")
                lua_selection = ""
                cmd = False

        # Twórz skrypt tylko gdy potrzebny (cmd=True)
        if cmd:
        ...
        if self.object_path is None:
            logger.error("Ścieżka do obiektu nie jest ustawiona")
            return
...
class Options(Exporter, gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        self.AddCheckbox(
            self.ui["CHK_AUTO_CLOSE"][0], c4d.BFH_SCALEFIT, 150, 10, "Auto Close"
        )
        self.GroupEnd()
        ...
        self.AddCheckbox(
            self.ui["CHK_NOMAT"][0], c4d.BFH_SCALEFIT, 150, 10, "No Materials"
        )
        self.AddCheckbox(
            self.ui["CHK_KEEP"][0], c4d.BFH_SCALEFIT, 150, 10, "Keep History"
        )
        self.GroupEnd()
        ...
        self.AddButton(
            self.ui_["BTN_CANCEL"], c4d.BFH_SCALEFIT, inith=15, name="Cancel"
        )
        self.GroupEnd()
    ...
    def Command(self, id, msg):
        if id == self.ui_["BTN_FIND"]:
            ...
            selected_file = c4d.storage.LoadDialog(
                type=c4d.FILESELECTTYPE_ANYTHING,
                title="RizomUV Executable",
            ...
            if selected_file:
                ...
                if os_name == "Windows":
                    ...
                    name = selected_file.replace("/Applications/", "").replace(
                        ".app", ""
                    )
                    abs_file_path = os.path.abspath(
                        selected_file + "/Contents/MacOS/" + name
                    )
...
class ScriptsManager(Exporter, gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        if self.scripts_folder is None or not os.path.exists(self.scripts_folder):
        ...
        self.AddCheckbox(
            self.ui["CHK_SINGLE"][0], c4d.BFH_LEFT, 150, 12, "Run and Close"
        )
        ...
        self.AddButton(
            self.ui_["BTN_RUN"], c4d.BFH_SCALEFIT, inith=15, initw=100, name="Run"
        )
        self.AddButton(
            self.ui_["BTN_CANCEL"], c4d.BFH_SCALEFIT, inith=15, initw=100, name="Close"
        )
        self.GroupEnd()
    ...
    def Command(self, id, msg):
        ...
        if id == self.ui_["BTN_NEW"]:
            ...
            if ids:
                c4d.gui.MessageDialog(
                    "The requested file could not be created because"
        ...
        if id == self.ui_["BTN_RUN"]:
            ...
            script_text = self.ui_get("text")
            if script_text and isinstance(script_text, str):
                Starter(script_text, cmd=True)
    ...
    def scan_folder(self):
        try:
            ...
            scripts = [
                file_ for file_ in os.listdir(self.scripts_folder) if file_.endswith(".lua")
            ]
            ...
            for index, name in enumerate(scripts):
                ...
                try:
                    ...
                    logger.warning(f"Nie można wczytać skryptu {name}: {str(e)}")
            ...
            logger.error(f"Błąd podczas skanowania folderu: {str(e)}")
            return False
        ...
        if self.scripts_folder:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.error(f"Błąd wczytywania skryptu {name}: {str(e)}")
                    return None
        return None
    ...
    def script_delete(self, name):
        ...
        if sub_dialog.answer and self.scripts_folder:
            file_path = os.path.join(self.scripts_folder, name)
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                    logger.info(f"Usunięto skrypt: {name}")
                except (OSError, IOError) as e:
                    error_msg = f"Nie można usunąć skryptu {name}: {str(e)}"
                    c4d.gui.MessageDialog(error_msg)
                    logger.error(error_msg)
...
class SubScriptDelete(c4d.gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        self.AddStaticText(
            1000, c4d.BFH_RIGHT, name="Are you sure want delete this script?"
        )
        self.GroupEnd()
...
def rizomuv_indexes(op, rizom_index=-1):
    ...
    # Sprawdź czy tag UV istnieje
    if tag is None:
        logger.warning(f"Obiekt {op.GetName()} nie ma tagu UV - pomijam")
        return {}
...
def json_save(path, data):
    try:
        ...
        with open(path, "w", encoding='utf-8') as file_:
            ...
            logger.info(f"Ustawienia zapisane: {path}")
            return True
    except (IOError, OSError) as e:
        logger.error(f"Błąd zapisu ustawień: {str(e)}")
        return False
...
def json_load(path):
    ...
    try:
        with open(path, "r", encoding='utf-8') as file_:
            logger.info(f"Ustawienia wczytane: {path}")
            return json.load(file_)
    except (IOError, OSError, ValueError) as e:
        logger.error(f"Błąd wczytywania ustawień: {str(e)}")
        return None
...
def fbx_config(fbx):
    ...
    if optionsUI.ui["CHK_NOMAT"][0]:
        if c4d.GetC4DVersion() < 22000:
            # FBXEXPORT_TEXTURES nie istnieje w nowszych wersjach C4D
            pass
        ...
        if mode == 0:
            ...
            try:
                if c4d.documents.SaveDocument(
                    doc_temp, obj_path, c4d.SAVEDOCUMENTFLAGS_DONTADDTORECENTLIST, 1026370
                ):
                    print("Exported to: " + obj_path)
                    logger.info(f"Eksport zakończony: {obj_path}")
                else:
                    error_msg = "Eksport nie powiódł się!"
                    gui.MessageDialog(error_msg)
                    logger.error(error_msg)
                    return False
            except Exception as e:
                error_msg = f"Błąd podczas eksportu: {str(e)}"
                gui.MessageDialog(error_msg)
                logger.error(error_msg)
                return False
...
if __name__ == "__main__":
    ...
    c4d.plugins.RegisterCommandPlugin(
        id=1041414,
        str="Export to RizomUV",
        help="Exchange to RizomUV, [Shift]: Open Options",
        info=0,
        dat=BCommandData("export"),
        icon=bmp_export,
    )
