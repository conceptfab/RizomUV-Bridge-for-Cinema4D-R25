...
class BCommandData(c4d.plugins.CommandData):
	...
	def Execute(self, doc):
		...
		c4d.gui.GetInputState(c4d.BFM_INPUT_KEYBOARD,
		...
		if self.plugin == 'export':
			...
			if bc[c4d.BFM_INPUT_QUALIFIER] == 1:
				dlg = Options()
				dlg.Open(c4d.DLG_TYPE_MODAL, xpos=-1,
						 ypos=-1, defaultw=200, defaulth=100)
			else:
				self.dlg = Starter()
			...
			dlg = Options()
			dlg.Open(c4d.DLG_TYPE_MODAL, xpos=-1,
					 ypos=-1, defaultw=400, defaulth=100)

		elif self.plugin == 'scripts':
			dlg = ScriptsManager()
			dlg.Open(c4d.DLG_TYPE_MODAL_RESIZEABLE, xpos=-
					 1, ypos=-1, defaultw=500, defaulth=250)
...
class Exporter:
	...
	def dispatch(self, mode):
		...
		if mode == 'roaming':
			self.plugin_folder = os.path.join(
				storage.GeGetC4DPath(c4d.C4D_PATH_PREFS), 'rizomUV')
	...
	def script_formation(self, selection, script):
		...
		if self.ui['CHK_SINGLE'][1] and len(script) > 0:
			save_string = 'ZomSave({File={Path="' + path + \
				'", UVWProps=true}, __UpdateUIObjFileName=true})'

		code = '{}\n{}\n{}\n{}'.format(
			load_string, selection, script, save_string)
	...
	def rizomuv_run(self, script, cmd):
		...
		if self.ui['RADIO_GROUP'][1] > 1:
			...
			for obj in self.selected_objs:
				...
				edges = [idx for idx, sel in enumerate(
					bs_e.GetAll(p_count * 4)) if sel]
		...
		try:
			...
			c4d.gui.MessageDialog(
				'RizomUV not found! Please configure path to rizomuv.exe in Option window first!')
		...
		wt = WatchThread('ht', self.doc, self.selected_objs,
						 self.object_path, self.time, self.p, self.ui)
		wt.start()
...
class Options(Exporter, gui.GeDialog):
	...
	def CreateLayout(self):
		...
		self.AddCheckbox(self.ui['CHK_NEW_UV'][0],
						 c4d.BFH_SCALEFIT, 150, 10, "New UVW")
		self.AddCheckbox(self.ui['CHK_AUTO_CLOSE'][0],
						 c4d.BFH_SCALEFIT, 150, 10, "Auto Close")
		self.GroupEnd()

		# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

		self.GroupBegin(4, c4d.BFH_SCALEFIT, 1, 2)
		self.AddCheckbox(self.ui['CHK_NOMAT'][0],
						 c4d.BFH_SCALEFIT, 150, 10, "No Materials")
		self.AddCheckbox(self.ui['CHK_KEEP'][0],
						 c4d.BFH_SCALEFIT, 150, 10, "Keep History")
		self.GroupEnd()
		...
		self.GroupBorderSpace(0, 10, 0, 10)
		self.AddStaticText(1500, c4d.BFH_SCALEFIT, 200,
						   10, "RizomUV Path: ", 0)
		...
		self.AddButton(self.ui_['BTN_SAVE'],
					   c4d.BFH_SCALEFIT, inith=15, name="Save")
		self.AddButton(self.ui_['BTN_CANCEL'],
					   c4d.BFH_SCALEFIT, inith=15, name="Cancel")
		self.GroupEnd()
	...
	def Command(self, id, msg):
		...
		if id == self.ui_['BTN_FIND']:
			...
			self.SetString(self.ui['TXT_U3D_PATH'][0],
						   self.ui['TXT_U3D_PATH'][1])
...
class ScriptsManager(Exporter, gui.GeDialog):
	...
	def CreateLayout(self):
		...
		self.AddComboBox(self.combo_box, c4d.BFH_SCALEFIT,
						 250, 15, specialalign=False)
		self.AddButton(self.ui_['BTN_UPD'],
					   c4d.BFH_LEFT, inith=15, name="Reload")
		...
		self.AddButton(self.ui_['BTN_SAVE'],
					   c4d.BFH_LEFT, inith=15, name="Save As")
		self.AddButton(self.ui_['BTN_DEL'],
					   c4d.BFH_LEFT, inith=15, name="Delete")
		...
		self.AddCheckbox(self.ui['CHK_SINGLE'][0],
						 c4d.BFH_LEFT, 150, 12, "Run and Close")
		...
		self.AddButton(self.ui_['BTN_RUN'], c4d.BFH_SCALEFIT,
					   inith=15, initw=100, name="Run")
		self.AddButton(self.ui_['BTN_CANCEL'], c4d.BFH_SCALEFIT,
					   inith=15, initw=100, name="Close")
		self.GroupEnd()
	...
	def scan_folder(self):
		...
		scripts = [file_ for file_ in os.listdir(
			self.scripts_folder) if file_.endswith('.lua')]
...
class SubScriptDelete(c4d.gui.GeDialog):
	...
	def CreateLayout(self):
		...
		self.AddStaticText(1000, c4d.BFH_RIGHT,
						   name='Are you sure want delete this script?')
		self.GroupEnd()
